    ...
    <div class="box">
        <div class="head">
            ...
            <div class="headuser">
                ...
                <!-- <el-dialog title="修改持卡人" :visible.sync="cardFormVisible">
                    <el-form>
                        <el-form-item label="持卡人" label-width="80px">
                            <el-input v-model="username" auto-complete="off" placeholder="请输入修改后的人名" label-width="80px"></el-input>
                        </el-form-item>
                    </el-form>
        ...
        <div class="contentbox">
            ...
            <div class="tablebox">
                <div class="table">
                    <div class="sumbox">
                        <span class="sumCard">卡片总数：</span>
                        <span class="sumNum">{{ sum_card }}</span>
                        <span class="sumPeople">人员总数：</span>
                        <span class="sumNum">{{ sum_people }}</span>        
                    </div>
                    <div class="tablebox1">
                        <el-table
                            ...
                            ref="filterTable"
                            height="100%"
                            :default-sort="{ prop:'card_id' , order: 'ascending' }"
                            ...
                            <el-table-column
                                ...
                                <template slot-scope="scope">
                                    <span style="margin-left: 10px">{{ scope.row.card_id }}</span>
                                </template>
                            </el-table-column>
                            ...
                            <el-table-column label="修改持卡人" 
                                width="250"
                                prop="update"
                            >
                                <template slot-scope="scope">
                                    <el-button type="primary" @click="updateCard(card_id=scope.row.card_id,username=scope.row.username)">修改</el-button>
                                </template>
                            </el-table-column>
                            ...
                            >
                                <template slot-scope="scope">
                                    <el-button type="danger" @click="deleteCard(card_id=scope.row.card_id)">删除</el-button>
                                </template>
                            </el-table-column>
...
export default{
    ...
    methods:{
        ...
        getTableData(){
            let that = this
            axios({
                url: '/LM.php?action=read_cards'
            }).then(res=>{
                let data =JSON.parse(JSON.stringify(res.data));
                let msg = data.cards;
                // let tableData = JSON.parse(JSON.stringify(res.data.users));
                // console.log(tableData);
                // this.tableData.push(msg.users)
                // console.log(tableData.users.id)
                // this.tableData.push(msg.users);

                // if (msg.users.length > 1000) {
                //         for (let n = 0,i = msg.users.length-1000; n < 1000,i < msg.users.length; n++,i++) {                      
                //             that.ListData[n] = msg.users[i]
                //         }                                   
                // }
                // else{
                //     for (let m = 0; m < msg.users.length; m++) {
                //         this.ListData[m] = msg.users[m];                      
                //     }
                // }
        ...
        sumadd(){
            this.sum_card = this.tableData.length
            
            let arr_key = []
            for (let i = 0; i <this.tableData.length; i++) {
                let key = this.tableData[i].username
                if(i > 0){
                    let index = arr_key.indexOf(key)
                    if(index != -1){
                        arr_key.splice(index,1)
                    }
                }
                arr_key.push(key) 
            }
            
            this.sum_people = arr_key.length
        ...
        updateCard(card_id,username){

            this.$prompt('请输入新持卡人', `修改${card_id}号卡`, {
                confirmButtonText: '确定',
                cancelButtonText: '取消',
                inputPlaceholder: '用户名由1-10位数字或字母组成',
                inputValue: username,
        ...
        deleteCard(card_id){
            this.$confirm(`此操作删除${card_id}号卡的所有信息, 是否继续?`, `删除`, {
                confirmButtonText: '确定',
                cancelButtonText: '取消',
                type: 'warning'
            }).then(() => {
